...
def append_data(file_name, max_len, user_index, origin_feature, user_feature, category_feature):
    ...
    for i in range(0, length):
        ...
        for j in range (3,size):
            origin_list[j] = str(max_len[j-3] + int(float(origin_list[j])) + 1) + ":1"
        ...
        s = len(user_list)
        for j in range(1, s):
            user_list[j] = str(max_len[j+size-4] + int(float(user_list[j])) + 1) + ":1"

        category_length = max_len[s+size-4]-max_len[s+size-5]
        category_list = list()

        for ca in category:
            category_list.append(str(max_len[s+size-4] + 1 + int(ca)) + ":1")
...
def write_file():
    ...
    #delete the userid and adid length
    new_len = temp_len[2:]
